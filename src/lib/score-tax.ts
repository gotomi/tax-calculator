    ...
    for (const month of yearSalary) {
        for (const key in summary) {
            if (typeof month[key as keyof SalaryData] === "number") {
                summary[key] = (summary[key] as number) + (month[key as keyof SalaryData] as number);
...
//   range: number[][],
//   GROSS_SALARY_LIMIT: number,
//   deductiblesLimit: number,
//   taxDeductibleCost: number,
//   authorTaxDeductibleCostRatio: number,
//   employer: boolean = false,
// ): any[][] {
//   const financialLimits = {
//     GROSS_SALARY_LIMIT: GROSS_SALARY_LIMIT,
//     deductiblesLimit: deductiblesLimit,
//     taxDeductibleCost: taxDeductibleCost,
//     authorTaxDeductibleCostRatio: authorTaxDeductibleCostRatio,
//   };
//   const grossSalaryPerMonthObj = range.flat();
//   const score = grossToNet(grossSalaryPerMonthObj, financialLimits, employer);

//   return score.map((item) => Object.values(item));
// }
